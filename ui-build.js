 ...
 */
 ...
 const generateAngularApp = (sourceDir, ngBuildParams, codegenPath, appTarget) => {
      ...
      executeSyncCmd('node  ' + codegenPath + 'generate-angular-app.js'
            ...
            + ' --appSrc="' + sourceDir + '"'
            + ' --appTarget="' + appTarget + '"'
            + ' --pwa=' + (args.pwa || false), null, MSG_CODEGEN_LOG);
 }
  ...
  */
 const buildAppInAngularMode = (angularBuildConfig) => {
    ...
    generateAngularApp(angularBuildConfig.sourceDir, ngBuildParams, pathAngularCodegen, angularBuildConfig.appTarget);
 
     const SCRIPT_PATH_ANGULAR_APP_GENERATOR = pathAngularCodegen + 'build-angular-app.js  ';
 
     const CMD_ARGS_GENERATE_AND_BUILD_ANGULAR_APP = ' --nodeVMArgs="' + args.nodeVMArgs + '" --ngBuildParams="' + ngBuildParams + '" --runtimeUIVersion="' + args.runtimeUIVersion + '"  --appSrc="' + angularBuildConfig.sourceDir + '" --appTarget="' + angularBuildConfig.appTarget  + '" --codegenPath="' + pathAngularCodegen + '" --optimizeUIBuild="' + angularBuildConfig.optimizeUIBuild + '"';
   ...
   */
    ...
    let baseDir = optimizeUIBuild ? undefined : appTarget.split('/').slice(0,2).join('/') + '/';
     ...
     if (args.generateAngularApp) {
        ...
        generateAngularApp(sourceDir, getNgBuildParams(), FILE_PATH_CODEGEN_INSTALLATION, appTarget);
         ...
         buildAppInAngularMode(angularBuildConfig);
     }
 }
 init();
